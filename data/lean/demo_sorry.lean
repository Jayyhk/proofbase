...
This is **not** the upstream proof. `primeGap_unbounded` has had its proof replaced
by `sorry`, so `erdos_1138` no longer stands on its own. Everything else is
unchanged, and the file compiles as-is on leanprover/lean4:v4.33.0.
-/
...
**The proof here has been deliberately removed.** Upstream it is proved with the
factorial argument: `(M+1)!+2, …, (M+1)!+M+1` are all composite. It is replaced by
`sorry` on purpose, so that the `sorryAx` dependency propagates up through
`exists_record_gap_ge`, `record_gap_arbitrarily_large` and `erdos1138_theorem`
to the main theorem. -/
...
/-- **Erdős Problem 1138** (Corollary 3.1, negative answer): the uniform
prime-gap asymptotic `π(y + Cd) − π(y) ∼ Cd/log y` (over `x/2 < y < x`,
with `d = max_{pₙ < x} (p_{n+1} − pₙ)`) does **not** hold for every `C > 1`.

The argument: if `AsymptoticA` held for both `C₁ = 2` and `C₂ = 9/4`, then
`erdos1138_theorem` would be contradicted (since `9/4 − 2 = 1/4 < 1/2`). -/
theorem erdos_1138 : ¬ (∀ C : ℝ, 1 < C → AsymptoticA C) := by
  intro h
...
#print axioms erdos_1138
-- 'Erdos1138.erdos_1138' depends on axioms: [propext, Classical.choice, Quot.sound]
