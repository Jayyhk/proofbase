...
lemma disjoint_A0_B0 : Disjoint A0 B0 := by
  ...
  have h_inj : Function.Injective toR2 := toR2_injective
...
lemma A0_not_on_a_line :
  ...
  have h_eq : ∀ x ∈ Aℤ, ∃ t : ℝ, toR2 x = p + t • (q - p) := by
    ...
    rw [SetLike.mem_coe, mem_affineSpan_pair_iff_exists_lineMap_eq] at hℓ_subset
    obtain ⟨r, hr⟩ := hℓ_subset
    refine ⟨r, ?_⟩
  ...
  obtain ⟨ x1, x2, x3, hx1, hx2, hx3, hx_not_collinear ⟩ : ∃ x1 x2 x3 : ℤ × ℤ, x1 ∈ Aℤ ∧ x2 ∈ Aℤ ∧ x3 ∈ Aℤ ∧ ¬(Collinear ℝ ({toR2 x1, toR2 x2, toR2 x3} : Set R2)) := by
    ...
    have ha0 := congr_fun (congrArg WithLp.ofLp ha) 0
    have hb0 := congr_fun (congrArg WithLp.ofLp hb) 0
    have hc0 := congr_fun (congrArg WithLp.ofLp hc) 0
    have ha1 := congr_fun (congrArg WithLp.ofLp ha) 1
    have hb1 := congr_fun (congrArg WithLp.ofLp hb) 1
    have hc1 := congr_fun (congrArg WithLp.ofLp hc) 1
    norm_num [toR2, WithLp.ofLp_toLp, vadd_eq_add] at *
    nlinarith;
...
#print axioms erdos_105
-- 'Erdos105.erdos_105' depends on axioms: [propext, Classical.choice, Quot.sound]
