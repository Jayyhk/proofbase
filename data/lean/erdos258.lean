...
Written against leanprover/lean4:v4.28.0, ported to v4.33.0.  Four changes:

* `convert ... using n` now emits instance-equality subgoals (for example
  `Real.instPreorder.toLE = Real.instLE`), so the two `convert` calls in
  `erdosTerm_summable` and `tail_geometric_bound` discharge those with `rfl`
  before the arithmetic tactic runs.
* `pow_succ'` is ambiguous under `open Nat`; the goal in `tail_geometric_bound`
  is over `ℝ`, so it is qualified as `_root_.pow_succ'`.
* With that resolved, the `norm_num` there closes more of the goal than it used
  to, so the follow-up steps are guarded with `all_goals`.
* `convert tsum_eq_sum_add_tsum_shift` in `erdos_258` now leaves the
  `erdosSeries` side; that side is definitionally the tsum, so the lemma is
...
lemma erdosTerm_pos (a : ℕ → ℕ) (ha : ∀ n, 0 < a (n + 1)) (n : ℕ) :
    0 < erdosTerm a n := by
  exact div_pos ( Nat.cast_pos.mpr <| Finset.card_pos.mpr ⟨ 1, by norm_num ⟩ ) <| Nat.cast_pos.mpr <| Q_pos a ha _
...
lemma erdosTerm_summable (a : ℕ → ℕ) (ha : ∀ n, 0 < a (n + 1))
  ...
  obtain ⟨N, hN⟩ : ∃ N, ∀ n ≥ N, 2 ≤ a (n + 1) := by
    exact Filter.eventually_atTop.mp ( ha_tendsto.eventually_ge_atTop 2 ) |> fun ⟨ N, hN ⟩ => ⟨ N, fun n hn => hN _ ( Nat.le_succ_of_le hn ) ⟩;
  have h_bound : ∀ n ≥ N, erdosTerm a n ≤ (n + 1 : ℝ) / (2 ^ (n - N)) / (Q a N : ℝ) := by
    have h_Q_bound : ∀ n ≥ N, Q a n ≥ 2 ^ (n - N) * Q a N := by
      intro n hn; induction hn <;> simp_all +decide ;
      rw [ Nat.succ_sub ( by linarith ), pow_succ' ] ; nlinarith [ hN _ ‹_›, pow_pos ( zero_lt_two' ℕ ) ( ‹_› - N ) ];
    intro n hn; rw [ erdosTerm ] ; rw [ div_div ] ; gcongr;
    ...
    · exact_mod_cast h_Q_bound ( n + 1 ) ( by linarith ) |> le_trans ( Nat.mul_le_mul_right _ ( pow_le_pow_right₀ ( by decide ) ( Nat.sub_le_sub_right ( Nat.le_succ _ ) _ ) ) );
  have h_pseries : Summable (fun n : ℕ => (n + 1 : ℝ) / 2 ^ n) := by
    refine' summable_of_ratio_norm_eventually_le _ _;
    exacts [ 3 / 4, by norm_num, Filter.eventually_atTop.mpr ⟨ 4, fun n hn => by rw [ Real.norm_of_nonneg ( by positivity ), Real.norm_of_nonneg ( by positivity ) ] ; rw [ div_le_iff₀ ( by positivity ) ] ; induction hn <;> norm_num [ pow_succ' ] at * ; ring_nf at * ; nlinarith ⟩ ];
  rw [ ← summable_nat_add_iff N ] at *;
  refine' .of_nonneg_of_le ( fun n => erdosTerm_nonneg a ha _ ) ( fun n => h_bound _ ( Nat.le_add_left _ _ ) ) _;
  convert h_pseries.mul_right ( 2 ^ N / ( Q a N : ℝ ) ) using 2 <;> try rfl
  norm_num ; try ring_nf;
...
lemma tsum_shift_pos {f : ℕ → ℝ} (hf : Summable f) (hf_pos : ∀ n, 0 < f n) (N : ℕ) :
    0 < ∑' n, f (N + n) := by
  exact lt_of_lt_of_le ( hf_pos _ ) ( Summable.le_tsum ( hf.comp_injective ( add_right_injective N ) ) 0 ( fun n hn => le_of_lt ( hf_pos _ ) ) )
...
lemma clearing_denominators_is_int (a : ℕ → ℕ) (ha : ∀ n, 0 < a (n + 1))
  ...
  have h1 : ∃ m : ℤ, ((r.den : ℝ) * (Q a N : ℝ) * (r : ℝ) = m) := by
    use r.num * Q a N;
    simp +decide [ mul_comm, mul_left_comm, Rat.cast_def ];
    rw [ ← mul_assoc, mul_div_cancel₀ _ ( Nat.cast_ne_zero.mpr r.pos.ne' ) ];
  have h2 : ∃ m : ℤ, ((r.den : ℝ) * (∑ n ∈ range N, (Q a N : ℝ) * erdosTerm a n) = m) := by
    have h_nat : ∀ n ∈ range N, ∃ m : ℕ, ((Q a N : ℝ) * erdosTerm a n = m) := by
      exact fun n hn => Q_mul_erdosTerm_nat a ha ( by linarith [ Finset.mem_range.mp hn ] );
    choose! m hm using h_nat;
    exact ⟨ r.den * ∑ n ∈ Finset.range N, m n, by push_cast; rw [ Finset.sum_congr rfl hm ] ⟩;
  obtain ⟨ m₁, hm₁ ⟩ := h1; obtain ⟨ m₂, hm₂ ⟩ := h2; use m₁ - m₂; simp_all +decide [ mul_sub ] ;
...
lemma tail_geometric_bound (a : ℕ → ℕ) (ha : ∀ n, 0 < a (n + 1))
    ...
    ↑B * ↑(Q a N) * (erdosSeries a - ∑ n ∈ range N, erdosTerm a n) < 1 / 2 := by
  have h_tail : erdosSeries a - ∑ n ∈ range N, erdosTerm a n = ∑' n, erdosTerm a (N + n) := by
    rw [ erdosSeries, ← Summable.sum_add_tsum_nat_add ];
    exacts [ by rw [ add_sub_cancel_left ] ; ac_rfl, hS ];
  have h_bound : ∑' n, erdosTerm a (N + n) ≤ ∑' n, (Λ / (M : ℝ)) ^ (n + 1) / (Q a N : ℝ) := by
    refine' Summable.tsum_le_tsum ( fun n => _ ) _ _;
    · have h_term_bound : ((N + n + 1).divisors.card : ℝ) / (Q a (N + n + 1) : ℝ) ≤ (Λ ^ (n + 1) : ℝ) / (Q a N * M ^ (n + 1)) := by
        gcongr;
        · exact mul_pos ( Nat.cast_pos.mpr ( Q_pos a ha N ) ) ( pow_pos ( Nat.cast_pos.mpr ( Nat.pos_of_ne_zero ( by rintro rfl; norm_num at hM; nlinarith ) ) ) _ );
        · grind +locals;
        · induction n with
          | zero => norm_num [ Nat.add_assoc, pow_succ, Q ] at *; exact mul_le_mul_of_nonneg_left ( mod_cast ha_large _ ( Nat.lt_succ_self _ ) ) ( Nat.cast_nonneg _ )
          | succ n ih => norm_num [ Nat.add_assoc, pow_succ, Q ] at *; nlinarith [ show ( M : ℝ ) ≤ a ( N + ( n + 2 ) ) by exact_mod_cast ha_large _ ( by linarith ), show ( 0 : ℝ ) ≤ Q a N * M ^ n by positivity, show ( 0 : ℝ ) ≤ Q a ( N + n ) * a ( N + ( n + 1 ) ) by positivity ]
      convert h_term_bound using 1 <;> first | rfl | ring!;
    · exact hS.comp_injective ( add_right_injective N );
    · exact Summable.mul_right _ ( summable_geometric_of_lt_one ( by positivity ) ( by rw [ div_lt_iff₀ ] <;> nlinarith ) |> Summable.comp_injective <| Nat.succ_injective );
  have h_geo_series : ∑' n, (Λ / (M : ℝ)) ^ (n + 1) / (Q a N : ℝ) = (Λ / (M : ℝ)) / (1 - Λ / (M : ℝ)) / (Q a N : ℝ) := by
    norm_num [ _root_.pow_succ', div_eq_mul_inv, tsum_mul_right ];
    all_goals exact Or.inl ( by erw [ ← tsum_geometric_of_lt_one ( by positivity ) ( by rw [ mul_inv_lt_iff₀ ( by norm_cast; linarith [ show M > 0 from Nat.pos_of_ne_zero ( by rintro rfl; norm_num at hM; nlinarith ) ] ) ] ; nlinarith ) ] ; all_goals erw [ ← tsum_mul_left ] ; all_goals exact tsum_congr fun _ => by ring );
  have h_final_bound : (B : ℝ) * (Q a N : ℝ) * ((Λ / (M : ℝ)) / (1 - Λ / (M : ℝ)) / (Q a N : ℝ)) < 1 / 2 := by
    field_simp;
    rw [ div_lt_iff₀ ] <;> nlinarith [ show ( 0 :ℝ ) < Q a N by exact mod_cast Q_pos a ha N, show ( 0 :ℝ ) < Λ * Q a N by exact mul_pos ( by positivity ) ( mod_cast Q_pos a ha N ), mul_div_cancel₀ Λ ( show ( M :ℝ ) ≠ 0 by norm_cast; rintro rfl; norm_num at hM; nlinarith ) ];
  exact h_tail.symm ▸ lt_of_le_of_lt ( mul_le_mul_of_nonneg_left h_bound <| by positivity ) ( by rw [ h_geo_series ] ; exact h_final_bound )
...
theorem tail_irrationality_lemma
  ...
  obtain ⟨N₁, hN₁⟩ : ∃ N₁ ≥ N₀, ∀ k > 0, ((N₁ + k).divisors.card : ℝ) ≤ Λ ^ k := by
    exact Frequently.forall_exists_of_atTop hbound N₀;
  obtain ⟨z, hz⟩ : ∃ z : ℤ, (z : ℝ) = B * Q a N₁ * (erdosSeries a - ∑ n ∈ range N₁, erdosTerm a n) := by
  ...
  have hz_pos : 0 < z := by
    have h_tail_pos : 0 < erdosSeries a - ∑ n ∈ range N₁, erdosTerm a n := by
      ...
      have h_tail_pos : erdosSeries a = ∑ n ∈ range N₁, erdosTerm a n + ∑' n, erdosTerm a (N₁ + n) := by
        exact tsum_eq_sum_add_tsum_shift (erdosTerm_summable a ha ha_tendsto) N₁;
      linarith;
    exact_mod_cast hz.symm ▸ mul_pos ( mul_pos ( Nat.cast_pos.mpr hB_pos ) ( Nat.cast_pos.mpr ( Q_pos a ha N₁ ) ) ) h_tail_pos;
  have hz_lt_half : B * Q a N₁ * (erdosSeries a - ∑ n ∈ range N₁, erdosTerm a n) < 1 / 2 := by
    apply tail_geometric_bound a ha (erdosTerm_summable a ha ha_tendsto) B hB_pos N₁ M Λ hΛ hM (fun n hn => hN₀ n (by linarith)) hN₁.right;
...
lemma divisors_card_le_two_pow_omega (n : ℕ) (hn : n ≠ 0) :
  ...
  exact Finset.prod_le_prod' fun p hp => by induction n.factorization p <;> simp_all +decide [ pow_succ' ] ; nlinarith;
...
/-- There exists `Λ > 1` such that for infinitely many `N`, `τ(N + k) ≤ Λ^k` for all `k ≥ 1`. -/
theorem tao_teravainen_divisors_le : ∃ Λ : ℝ, 1 < Λ ∧
  ...
  · exact hC.mono fun N hN k hk => by
      ...
      calc ((N + k).divisors.card : ℝ)
          ≤ ((2 ^ Ω : ℕ) : ℝ) := by exact_mod_cast divisors_card_le_two_pow_omega _ hNk_ne
        _ = (2 : ℝ) ^ (Ω : ℕ) := by push_cast; ring
        _ = (2 : ℝ) ^ (Ω : ℝ) := (Real.rpow_natCast 2 Ω).symm
        _ ≤ (2 : ℝ) ^ (C * k) := Real.rpow_le_rpow_of_exponent_le (by norm_num) (by exact_mod_cast hΩ)
...
-- 'Erdos258.erdos_258' depends on axioms: [propext, Classical.choice, Erdos258.tao_teravainen, Quot.sound]
