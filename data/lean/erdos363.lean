...
theorem intervals_disjoint (n : ℕ) (h : n > 1) :
  ...
  · grind;
  · grind;
  · intros; nlinarith [Nat.sub_add_cancel (show 1 ≤ 7 * n + 4 * n ^ 2 by nlinarith)]
...
/-
A collection of intervals is valid if all elements are intervals, each has size >= 4, they are pairwise disjoint, and the product of all elements in all intervals is a square.
-/
...
theorem collection_is_valid (n : ℕ) (h : n > 1) : is_valid_collection (collection n) := by
  refine ⟨?_, ?_, ?_, ?_⟩
  · -- every element is an interval Icc a b
    intro I hI; fin_cases hI <;> exact ⟨_, _, rfl⟩
  · -- every interval has at least 4 elements
    intro I hI; fin_cases hI <;> simp only [I₁, I₂, I₃, I₄, Nat.card_Icc] <;> omega
  · -- the intervals are pairwise disjoint
    norm_num [ collection, intervals_disjoint n h ]
  · -- the total product is a square
    refine ⟨ root_term n, ?_ ⟩
    have h2 := product_is_square n h
    simp only [collection, List.map_cons, List.map_nil, List.prod_cons, List.prod_nil, mul_one]
    rw [pow_two] at h2
