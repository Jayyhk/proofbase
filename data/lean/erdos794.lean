...
def has_subgraph (V : Finset ℕ) (E : Finset (Finset ℕ)) (v_count e_count : ℕ) : Prop :=
  ∃ s, s ⊆ V ∧ s.card = v_count ∧ (E.filter (fun e => e ⊆ s)).card ≥ e_count

set_option maxRecDepth 4000 in
theorem counterexample_disproves_conjecture :
  (∀ e ∈ counterexample_edges, e ⊆ V_set ∧ e.card = 3) ∧
  counterexample_edges.card ≥ n_val^3 + 1 ∧
  ¬ has_subgraph V_set counterexample_edges 4 3 ∧
  ¬ has_subgraph V_set counterexample_edges 5 7 := by
    unfold has_subgraph;
    decide

def conjecture_794 : Prop :=
  ∀ (n : ℕ) (V : Finset ℕ) (E : Finset (Finset ℕ)),
    V.card = 3 * n →
    (∀ e ∈ E, e ⊆ V ∧ e.card = 3) →
    E.card ≥ n^3 + 1 →
      has_subgraph V E 4 3 ∨ has_subgraph V E 5 7

theorem erdos_794 : ¬conjecture_794 := by
  intro h
  have hce := counterexample_disproves_conjecture
  have hV : V_set.card = 3 * n_val := by decide
  have hinst :
      has_subgraph V_set counterexample_edges 4 3 ∨
      has_subgraph V_set counterexample_edges 5 7 :=
    h n_val V_set counterexample_edges hV hce.1 hce.2.1
  simp_all only [ge_iff_le, or_self]

#print axioms erdos_794
